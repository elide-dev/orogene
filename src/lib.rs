...
use std::{borrow::Cow, panic::PanicHookInfo, sync::Arc};
use std::{
    ...
    fn first_time_setup(&mut self) -> Result<()> {
        if let Some(config_path) = self.config.clone().or_else(|| {
            ...
            if self.first_time && !is_ci::cached() {
                ...
                let telemetry_exists = if config.get("options").is_none() {
                    ...
                    config
                        ...
                        .children()
                        .as_ref()
                        .map(|doc| doc.get("telemetry").is_some())
                        .unwrap_or(false)
                };
                ...
                if config.get("options").unwrap().get("first-time").is_none() {
